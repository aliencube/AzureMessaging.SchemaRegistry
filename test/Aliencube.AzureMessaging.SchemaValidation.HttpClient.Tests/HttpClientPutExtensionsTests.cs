...
{
    [TestClass]
    public class HttpClientPutExtensionsTests
    {
        ...
        public void Given_Type_Then_It_Should_Have_Methods()
        {
            ...
            methods
                .Should().Contain(p => p.Name.IsEquivalentTo("PutAsync", StringComparison.CurrentCulture))
                ;
        }
        ...
        public void Given_Validation_Error_When_PutAsync_Invoked_Then_It_Should_Throw_Exception(HttpStatusCode statusCode)
        {
            var requestUri = "http://localhost";
        ...
        [TestMethod]
        public void Given_Null_Parameters_With_CancellationToken_When_PutAsync_Invoked_Then_It_Should_Throw_Exception()
        {
            var requestUri = "http://localhost";
            var validator = new Mock<ISchemaValidator>();
            var path = "default.json";
            ...
            using (var content = this._fixture.CreateHttpContent())
            {
                func = async () => await HttpClientExtensions.PutAsync(null, (string)null, null, null, null, token).ConfigureAwait(false);
                func.Should().Throw<ArgumentNullException>();

                func = async () => await HttpClientExtensions.PutAsync(httpClient, (string)null, null, null, null, token).ConfigureAwait(false);
                func.Should().Throw<ArgumentNullException>();

                func = async () => await HttpClientExtensions.PutAsync(httpClient, requestUri, null, null, null, token).ConfigureAwait(false);
                func.Should().Throw<ArgumentNullException>();

                func = async () => await HttpClientExtensions.PutAsync(httpClient, requestUri, content, null, null, token).ConfigureAwait(false);
                func.Should().Throw<ArgumentNullException>();

                func = async () => await HttpClientExtensions.PutAsync(httpClient, requestUri, content, validator.Object, null, token).ConfigureAwait(false);
                func.Should().Throw<ArgumentNullException>();

                func = async () => await HttpClientExtensions.PutAsync(httpClient, requestUri, content, validator.Object, path, token).ConfigureAwait(false);
                func.Should().Throw<ArgumentNullException>();

                func = async () => await HttpClientExtensions.PutAsync(null, (Uri)null, null, null, null, token).ConfigureAwait(false);
                func.Should().Throw<ArgumentNullException>();

                func = async () => await HttpClientExtensions.PutAsync(httpClient, (Uri)null, null, null, null, token).ConfigureAwait(false);
                func.Should().Throw<ArgumentNullException>();

                func = async () => await HttpClientExtensions.PutAsync(httpClient, new Uri(requestUri), null, null, null, token).ConfigureAwait(false);
                func.Should().Throw<ArgumentNullException>();

                func = async () => await HttpClientExtensions.PutAsync(httpClient, new Uri(requestUri), content, null, null, token).ConfigureAwait(false);
                func.Should().Throw<ArgumentNullException>();

                func = async () => await HttpClientExtensions.PutAsync(httpClient, new Uri(requestUri), content, validator.Object, null, token).ConfigureAwait(false);
                func.Should().Throw<ArgumentNullException>();

                func = async () => await HttpClientExtensions.PutAsync(httpClient, new Uri(requestUri), content, validator.Object, path, token).ConfigureAwait(false);
                func.Should().Throw<ArgumentNullException>();
            }
        }
        ...
        [DataTestMethod]
        [DataRow(HttpStatusCode.BadRequest)]
        public void Given_Error_Response_With_CancellationToken_When_PutAsync_Invoked_Then_It_Should_Throw_Exception(HttpStatusCode statusCode)
        {
            var requestUri = "http://localhost";
            var validator = new Mock<ISchemaValidator>();
            var path = "default.json";
        ...
        [DataTestMethod]
        [DataRow(HttpStatusCode.OK)]
        public async Task Given_Validation_Result_With_CancellationToken_When_PutAsync_Invoked_Then_It_Should_Return_Result(HttpStatusCode statusCode)
        {
            var requestUri = "http://localhost";
            var payload = "{ \"hello\": \"world\" }";
            ...
            using (var content = this._fixture.CreateHttpContent())
            using (var source = this._fixture.CreateCancellationTokenSource())
            using (var result1 = await HttpClientExtensions.PutAsync(httpClient, requestUri, content, validator.Object, path, source.Token).ConfigureAwait(false))
            using (var result2 = await HttpClientExtensions.PutAsync(httpClient, new Uri(requestUri), content, validator.Object, path, source.Token).ConfigureAwait(false))
            {
                result1.Should().Be(response);
                result2.Should().Be(response);
            }
